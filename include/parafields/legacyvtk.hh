...
public:
   ...
   * @param comm     MPI communicator, used to abort if field is distributed in
   * parallel
   */
  LegacyVTKWriter(const Dune::ParameterTree& config,
                  const std::string& fileName,
                  const MPI_Comm& comm = MPI_COMM_WORLD)
    : file(fileName + ".vtk", std::ofstream::trunc)
    , preambleWritten(false)
    , extensions(config.template get<std::vector<typename Traits::RF>>(
        "grid.extensions"))
