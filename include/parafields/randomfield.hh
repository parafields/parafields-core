 ...
 * @tparam IsoMatrix   covariance matrix implementation using underlying
 * symmetries
 * @tparam AnisoMatrix covariance matrix implementation for general covariance
 * functions
 */
...
protected:
  ...
  public:
     ...
     */
    const Dune::ParameterTree& get() const { return config; }
  };
...
public:
  ...
  explicit RandomField(const Dune::ParameterTree& config_,
                       const std::string& fileName = "",
                       const LoadBalance& loadBalance = LoadBalance(),
                       const MPI_Comm comm = MPI_COMM_WORLD)
    : config(config_)
    , valueTransform(config)
    , traits(new Traits(config, loadBalance, comm))
    , trendPart(config, traits, fileName)
    , stochasticPart(traits, fileName)
    , cacheInvMatvec((*traits).cacheInvMatvec)
    , cacheInvRootMatvec((*traits).cacheInvRootMatvec)
    ...
    , invRootMatvecValid(false)
  {
    ...
    if (cacheInvMatvec)
      invMatvecPart = std::shared_ptr<StochasticPartType>(
        new StochasticPartType(stochasticPart));

    if (cacheInvRootMatvec)
      invRootMatvecPart = std::shared_ptr<StochasticPartType>(
        new StochasticPartType(stochasticPart));
  }
   ...
   *
   * This constructor reads not only the field, but also its underlying
   * parameters (i.e., the ParameterTree config object) from a file. This can be
   * used to read in a field that was generated by another program run or
   * another program altogether, and still have the correct parameterization
   * available.
   *
   * @tparam LoadBalance class used for parallel data distribution with MPI
   *
   ...
   */
  template<typename LoadBalance = DefaultLoadBalance<GridTraits::dim>>
  RandomField(const std::string& fileName,
              const LoadBalance& loadBalance = LoadBalance(),
              const MPI_Comm comm = MPI_COMM_WORLD)
    : treeHelper(fileName)
    , config(treeHelper.get())
    , valueTransform(config)
    , traits(new Traits(config, loadBalance, comm))
    , trendPart(config, traits, fileName)
    , stochasticPart(traits, fileName)
   ...
   * This avoids generating the same covariance matrix several times, saving
   * memory when multiple fields have to be read in and subsequently modified.
   *
  ...
  RandomField(const RandomField& other, const GFS& gfs, const Field& field)
    ...
    , anisoMatrix(other.anisoMatrix)
    , useAnisoMatrix(other.useAnisoMatrix)
    , trendPart(other.trendPart, gfs, field)
    , stochasticPart(other.stochasticPart, gfs, field)
    , cacheInvMatvec((*traits).cacheInvMatvec)
    , cacheInvRootMatvec((*traits).cacheInvRootMatvec)
    , invMatvecValid(false)
    , invRootMatvecValid(false)
  ...
  RandomField(const RandomField& other, const DGF& dgf)
    ...
    , anisoMatrix(other.anisoMatrix)
    , useAnisoMatrix(other.useAnisoMatrix)
    , trendPart(other.trendPart, dgf)
    , stochasticPart(other.stochasticPart, dgf)
    , cacheInvMatvec((*traits).cacheInvMatvec)
    , cacheInvRootMatvec((*traits).cacheInvRootMatvec)
    , invMatvecValid(false)
    , invRootMatvecValid(false)
    ...
    , invRootMatvecValid(other.invRootMatvecValid)
  {
    if (cacheInvMatvec && other.cacheInvMatvec)
      invMatvecPart = std::shared_ptr<StochasticPartType>(
        new StochasticPartType(*(other.invMatvecPart)));

    if (cacheInvRootMatvec && other.cacheInvRootMatvec)
      invRootMatvecPart = std::shared_ptr<StochasticPartType>(
        new StochasticPartType(*(other.invRootMatvecPart)));
  }
  ...
  void evaluate(const Element& elem,
  ...
  /**
   * @brief Export random field as flat unstructured VTK file, requires
   * dune-grid and dune-functions
   *
  ...
  /**
   * @brief Export random field as unstructured VTK file, requires dune-grid and
   * dune-functions
   *
  ...
  void writeToVTKSeparate(const std::string& fileName, const GridView& gv) const
  {
    ...
    Dune::VTKWriter<GridView> vtkWriter(gv, Dune::VTK::conforming);
    {
      auto f = Dune::Functions::makeAnalyticGridViewFunction(
        [&](auto x) {
          typename Traits::RangeType output;
          stochasticPart.evaluate(x, output);
          return output;
        },
        gv);
      vtkWriter.addCellData(
        f, Dune::VTK::FieldInfo("stochastic", Dune::VTK::FieldInfo::Type::scalar, 1));
    }
    for (unsigned int i = 0; i < trendPart.size(); i++) {
      ...
      auto f = Dune::Functions::makeAnalyticGridViewFunction(
        [&](auto x) {
          typename Traits::RangeType output;
          component.evaluate(x, output);
          return output;
        },
        gv);
      vtkWriter.addCellData(
        f, Dune::VTK::FieldInfo(component.name(), Dune::VTK::FieldInfo::Type::scalar, 1));
    }
    vtkWriter.pwrite(fileName, "", "", Dune::VTK::appendedraw);
#else  // HAVE_DUNE_FUNCTIONS
    throw std::runtime_error{
   ...
   *
   * Same as writeToVTK, but writes a simple legacy VTK format and doesn't
   * depend on dune-grid and dune-functions. Unfortunately, this function
   ...
   * doesn't depend on dune-grid and dune-functions. Unfortunately, this
   * function doesn't work for parallel runs, only for sequential field
   * creation.
   *
   * @param fileName file name for VTK output
   *
  ...
  {
    return (trendPart == other.trendPart &&
            stochasticPart == other.stochasticPart);
  }
 ...
 *
 * This class represents a list of uncorrelated stationary Gaussian random
 * fields. Each such field has its own variance, covariance function, etc.
 * Fields can be added to the list one by one, and the field list supports most
 * operations the individual fields do, so this class can be used as a drop-in
 * when multiple fields have to be managed at the same time, i.e., in joint
 * Bayesian inversion of multiple parameterizations.
 *
 * @tparam GridTraits  class defining dimension and data type of entries, etc.
 * @tparam IsoMatrix   covariance matrix implementation using underlying
 * symmetries
 * @tparam AnisoMatrix covariance matrix implementation for general covariance
 * functions
 * @tparam RandomField type of the field entries, can be a subtype of
 * RandomField
 */
template<typename GridTraits,
         template<typename> class IsoMatrix =
...
public:
  ...
  RandomFieldList(const Dune::ParameterTree& config_,
                  const std::string& fileName = "",
                  const LoadBalance loadBalance = LoadBalance(),
                  const MPI_Comm comm = MPI_COMM_WORLD)
    : config(config_)
  {
    ...
    while (std::getline(typeStream, type, ' ')) {
      ...
      if (!subConfig.hasKey("grid.extensions") &&
          config.hasKey("grid.extensions"))
      ...
      if (!subConfig.hasKey("randomField.cgIterations") &&
        ...
        subConfig["randomField.cgIterations"] =
          config["randomField.cgIterations"];
      ...
      list.insert({ type,
                    std::make_shared<SubRandomField>(
                      subConfig, subFileName, loadBalance, comm) });
    }
   ...
   */
  template<typename LoadBalance = DefaultLoadBalance<Traits::dim>>
  RandomFieldList(const std::string& fileName,
                  const LoadBalance loadBalance = LoadBalance(),
                  const MPI_Comm comm = MPI_COMM_WORLD)
    : treeHelper(fileName)
    , config(treeHelper.get())
  {
    ...
    while (std::getline(typeStream, type, ' ')) {
      ...
      list.insert(
        { type,
          std::make_shared<SubRandomField>(subFileName, loadBalance, comm) });
    }
  ...
  /**
   * @brief Constructor reading random fields from file, but reusing covariance
   * matrices
   *
   * This constructor also reads the fiels from file, but reuses the
   * configuration and covariance matrices from another field list that has
   * already been constructed. This avoids generating the same covariance
   * matrices several times, saving memory when multiple fields have to be read
   * in and subsequently modified.
   *
  ...
  {
    for (const std::pair<std::string, std::shared_ptr<SubRandomField>>& pair :
         other.list)
      list.insert({ pair.first,
                    std::make_shared<SubRandomField>(
                      *(pair.second), fileName + "." + pair.first) });
  }
   ...
   * the random fields. Useful to convert sensitivities during Bayesian
   * inversion.
   *
   * @tparam GFS   type of GridFunctionSpace
   * @tparam Field type of coefficient vector
    ...
    , activeTypes(other.activeTypes)
  {
    for (const std::string& type : activeTypes) {
      ...
      list.insert({ type,
                    std::make_shared<SubRandomField>(
                      *otherField, gfs, *(fieldList.find(type)->second)) });
    }
   ...
   *
   * This constructor uses a map of PDELab DiscreteGridFunctions
   * (representations of functions living on a finite element mesh) to
   * initialize the values of a random field list. Useful to convert
   * sensitivities during Bayesian inversion.
   *
   * @tparam DGF   type of DiscreteGridFunction
   *
   * @param other other random field list to copy most members from
    ...
    , activeTypes(other.activeTypes)
  {
    for (const std::string& type : activeTypes) {
      ...
      list.insert({ type,
                    std::make_shared<SubRandomField>(
                      *otherField, *(dgfList.find(type)->second)) });
    }
    ...
    , activeTypes(other.activeTypes)
  {
    for (const std::pair<std::string, std::shared_ptr<SubRandomField>>& pair :
         other.list)
      list.insert(
        { pair.first, std::make_shared<SubRandomField>(*(pair.second)) });
  }
  ...
  /**
   * @brief Define subset of fields kept constant (i.e., not changed by
   * mathematical operators)
   *
   * Activate the given number of fields, in the order they were added to the
   * list. Activated fields participate in mathematical operations and are
   * changed when another field list is added, subtracted, etc. If this number
   * is smaller than the number of fields that are stored, the remaining fiels
   * are kept constant and will not be modified by such operations.
   *
   ...
   * In an optimization problem based on random field lists, this is the
   * dimension of the parameter space, since the coefficients of the inactive
  ...
  /**
   * @brief Export random fields as flat unstructured VTK files, requires
   * dune-grid and dune-functions
   *
  ...
  /**
   * @brief Export random fields as unstructured VTK files, requires dune-grid
   * and dune-functions
   *
