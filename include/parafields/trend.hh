...
public:
   ...
   */
  TrendComponent(const std::shared_ptr<Traits>& traits_,
  ...
  void construct(const GFS& gfs, const Field& field)
  {
    ...
    for (const auto& elem :
         elements(gfs.gridView(), Dune::Partitions::interior)) {
      lfs.bind(elem);
    ...
    MPI_Allreduce(&(myNewShiftVector[0]),
                  &(newShiftVector[0]),
                  shiftVector.size(),
                  mpiType<RF>,
                  MPI_SUM,
   ...
   * @return reference to updated trend component
   */
  TrendComponent<Traits>& axpy(const TrendComponent<Traits>& other,
                               const RF alpha)
  {
    for (unsigned int i = 0; i < shiftVector.size(); ++i)
                ...
                typename Traits::RangeType& output) const
  {
    if (TrendComponentType::isMean(componentType)) {
      ...
      for (unsigned int i = 0; i < dim; i++)
        output[0] +=
          (meanVector[i] + shiftVector[i]) * (location[i] - extensions[i] / 2.);
    } else if (TrendComponentType::isDisk(componentType)) {
      output[0] = 0.;
      ...
      for (unsigned int i = 0; i < dim; i++)
        distSquared +=
          std::pow(location[i] - (meanVector[i] + shiftVector[i]), 2);
 ...
 */
template<typename Traits>
class ImageComponent : public TrendComponent<Traits>
{
  using RF = typename TrendComponent<Traits>::RF;

  enum
  {
    dim = TrendComponent<Traits>::dim
  };
...
public:
   ...
   */
  template<typename GFS, typename Field>
  ImageComponent(const ImageComponent<Traits>& other,
                 const GFS& gfs,
                 const Field& field)
...
public:
   ...
   */
  TrendPart(const Dune::ParameterTree& config,
    ...
    : traits(traits_)
  {
    ...
    if (!meanVector.empty()) {
      ...
      componentVector.emplace_back(traits,
                                   trendVector,
                                   meanVector,
                                   varianceVector,
                                   TrendComponentType::Mean);
    }
    ...
    while (!endReached) {
      ...
      s << count;
      meanVector =
        config.get<std::vector<RF>>("disk" + s.str() + ".mean", emptyVector);

      if (meanVector.empty()) {
        endReached = true;
      } else {
        varianceVector =
          config.get<std::vector<RF>>("disk" + s.str() + ".variance");
    ...
    while (!endReached) {
      ...
      s << count;
      meanVector =
        config.get<std::vector<RF>>("block" + s.str() + ".mean", emptyVector);

      if (meanVector.empty()) {
        endReached = true;
      } else {
        varianceVector =
          config.get<std::vector<RF>>("block" + s.str() + ".variance");
   ...
   */
  template<typename GFS, typename Field>
  TrendPart(const TrendPart<Traits>& other, const GFS& gfs, const Field& field)
    : traits(other.traits)
    , componentVector(other.componentVector)
  {
    ...
    if (other.imageComponent) {
      imageComponent =
        std::make_shared<ImageComponent<Traits>>(*(other.imageComponent));
  ...
  /**
   * @brief Multiply the trend part with approximate inverse root of its cov.
   * matrix
   *
   * This function performs multiplication with the inverse square root
   * of the covariance matrix.
   ...
   * @param[out] output value of trend component at given coordinates
   */
  void evaluate(const typename Traits::DomainType& x,
                typename Traits::RangeType& output) const
  {
    output = 0.;
